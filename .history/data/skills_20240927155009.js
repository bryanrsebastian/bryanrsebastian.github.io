// https://www.svgrepo.com/

export default {
  "tools" : {
    "mongodb" : {
      "title" : "MongoDB",
      "img" : "/imgs/skill-logos/mongodb.svg",
      "percent" : "80",
      "color" : "#81c784"
    },
    "expressjs" : {
      "title" : "ExpressJS",
      "img" : "/imgs/skill-logos/express.png",
      "percent" : "85",
      "color" : "#c6c5c5"
    },
    "html" : {
      "title" : "HTML",
      "img" : "/imgs/skill-logos/html5.svg",
      "percent" : "95",
      "color" : "#ffa968"
    },
    "reactjs" : {
      "title" : "ReactJS",
      "img" : "/imgs/skill-logos/reactjs.svg",
      "percent" : "60",
      "color" : "#00d8fe"
    },
    "nodejs" : {
      "title" : "NodeJS",
      "img" : "/imgs/skill-logos/nodejs.svg",
      "percent" : "80",
      "color" : "#69b16c"
    },
    "css" : {
      "title" : "CSS",
      "img" : "/imgs/skill-logos/css3.svg",
      "percent" : "95",
      "color" : "#39beff"
    },
    "sass" : {
      "title" : "SASS",
      "img" : "/imgs/skill-logos/sass.svg",
      "percent" : "85",
      "color" : "#ff97ba"
    },
    "bootstrap" : {
      "title" : "Bootstrap",
      "img" : "/imgs/skill-logos/bootstrap.svg",
      "percent" : "85",
      "color" : "#c09cff"
    },
    "foundation" : {
      "title" : "Foundation",
      "img" : "/imgs/skill-logos/foundation.svg",
      "percent" : "90",
      "color" : "#76dfff"
    },
    "js" : {
      "title" : "JavaScript",
      "img" : "/imgs/skill-logos/javascript.svg",
      "percent" : "75",
      "color" : "#ffec87"
    },
    "json" : {
      "title" : "JSON",
      "img" : "/imgs/skill-logos/json.png",
      "percent" : "85",
      "color" : "#6fd1ff"
    },
    "jquery" : {
      "title" : "jQuery",
      "img" : "/imgs/skill-logos/jquery.png",
      "percent" : "90",
      "color" : "#2d7cc6"
    },
    "npm" : {
      "title" : "NPM",
      "img" : "/imgs/skill-logos/npm.svg",
      "percent" : "85",
      "color" : "#dc3b3b"
    },
    "vuejs" : {
      "title" : "VueJS",
      "img" : "/imgs/skill-logos/vuejs.svg",
      "percent" : "75",
      "color" : "#9dcc9f"
    },
    "nextjs" : {
      "title" : "NextJS",
      "img" : "/imgs/skill-logos/nextjs.svg",
      "percent" : "60",
      "color" : "#ffffff"
    },
    "php" : {
      "title" : "PHP",
      "img" : "/imgs/skill-logos/php.svg",
      "percent" : "95",
      "color" : "#c4b3fb"
    },
    "mysql" : {
      "title" : "MySQL",
      "img" : "/imgs/skill-logos/mysql.svg",
      "percent" : "85",
      "color" : "#ffa82b"
    },
    "wordpress" : {
      "title" : "WordPress",
      "img" : "/imgs/skill-logos/wordpress.svg",
      "percent" : "95",
      "color" : "#2c71a7"
    },
    "git" : {
      "title" : "Git",
      "img" : "/imgs/skill-logos/git.svg",
      "percent" : "90",
      "color" : "#f77e58"
    },
    "photoshop" : {
      "title" : "PhotoShop",
      "img" : "/imgs/skill-logos/photoshop.svg",
      "percent" : "75",
      "color" : "#66e9ff"
    },
    "xd" : {
      "title" : "Experience Design",
      "img" : "/imgs/skill-logos/xd.svg",
      "percent" : "50",
      "color" : "#de73ff"
    },
    "linux" : {
      "title" : "Linux Server",
      "img" : "/imgs/skill-logos/linux.png",
      "percent" : "70",
      "color" : "#ffd659"
    },
  },
  "capabilities" : [
    "Scheduling a task or even a whole project when will it be done.",
    "Mentoring my colleagues if they are new with the tools, especially the junior dev.",
    "Talking to a client if the project manager does not know how to answer some technical questions.",
    "Assessing some functionalities if that is feasible.",
    "Assisting my colleagues if they don't know how to do a conflict task.",
    "Creating a clean code with a good comment and eye-catching indention.",
    "Documenting my functionalities, modules, plugins, or API to use by other devs.",
    "Leading a team on how to manage difficult projects and distribute a task to my team member."
  ]
}